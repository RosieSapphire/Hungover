...
/*
static void _meshes_read(scene_t *s, FILE *file)
{
	fread(&s->num_meshes, sizeof(uint16_t), 1, file);
	s->num_meshes = uint16_endian_flip(s->num_meshes);
	printf("num_meshes=%d\n", s->num_meshes);
	s->meshes = malloc(sizeof(mesh_t) * s->num_meshes);
	for(int i = 0; i < s->num_meshes; i++) {
		mesh_t *mesh = s->meshes + i;
...
static void _anims_read(scene_t *s, FILE *file)
{
	fread(&s->num_anims, sizeof(uint16_t), 1, file);
	s->num_anims = uint16_endian_flip(s->num_anims);
	printf("num_anims=%d\n", s->num_anims);
	s->anims = malloc(sizeof(animation_t) * s->num_anims);
	for(int i = 0; i < s->num_anims; i++) {
		animation_t *anim = s->anims + i;
		fread(&anim->name, sizeof(char), NAME_MAX_LEN, file);
		fread(&anim->mesh_index, sizeof(uint16_t), 1, file);
		anim->mesh_index = uint16_endian_flip(anim->mesh_index);
		...
		printf("\tname=%s, length=%d, mesh_index=%d\n"
				"\t\tnum_pos=%d, num_rot=%d, num_sca=%d\n",
				anim->name, anim->length, anim->mesh_index,
				anim->num_pos, anim->num_rot, anim->num_sca);
...
static void _node_read(scene_t *s, node_t *n, FILE *file, uint16_t depth)
{
	...
	printf("\tname=%s, mesh_index=%d, num_children=%d\n", n->name,
			uint16_endian_flip(n->mesh_index),
			uint16_endian_flip(n->num_children));
	for(int i = 0; i < uint16_endian_flip(n->num_children); i++)
		_node_read(s, n->children + i, file, depth + 1);
}

static void _read_test(const char *path)
{
	...
	_meshes_read(&scene, file);
	_anims_read(&scene, file);
	printf("nodes\n");
	_node_read(&scene, &scene.root_node, file, 0);
	fclose(file);
}
*/
...
static node_t _node_process(const struct aiNode *n)
{
	...
	node.children = malloc(sizeof(node_t) * node.num_children);
