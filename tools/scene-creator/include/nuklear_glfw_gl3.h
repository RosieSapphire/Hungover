...
NK_API void nk_glfw3_render(enum nk_anti_aliasing, int max_vertex_buffer,
...
NK_API void nk_glfw3_render(enum nk_anti_aliasing AA, int max_vertex_buffer,
			    int max_element_buffer)
{
	struct nk_glfw_device *dev = &glfw.ogl;
	...
	glViewport(0, 0, (GLsizei)glfw.display_width,
		   (GLsizei)glfw.display_height);
	{
		...
		elements = glMapBuffer(GL_ELEMENT_ARRAY_BUFFER, GL_WRITE_ONLY);
		{
			...
			static const struct nk_draw_vertex_layout_element
				vertex_layout[] = {
					{ NK_VERTEX_POSITION, NK_FORMAT_FLOAT,
					  NK_OFFSETOF(struct nk_glfw_vertex,
						      position) },
			...
			nk_buffer_init_fixed(&ebuf, elements,
					     (size_t)max_element_buffer);
			nk_convert(&glfw.ctx, &dev->cmds, &vbuf, &ebuf,
				   &config);
		}
